...
class AppData: ObservableObject {
    ...
    @Published var academicYears: [AcademicYear] = [] {
        didSet { save() }
    }
    ...
    var body: some View {
        NavigationSplitView {
            ...
            if selection == "Général" {
                ...
                ParcoursMainView(appData: appData)
                ...
                Text("Sélectionne un élément dans le menu").foregroundColor(.secondary)
            }
        }
...
struct GeneralView: View {
    ...
    var body: some View {
            ...
            VStack(alignment: .leading, spacing: 30) {
                ...
                HStack {
                    ...
                    if appData.getDisplayStreak() > 0 {
                        HStack(spacing: 4) {
                            Text("🔥")
                            Text("\(appData.getDisplayStreak()) jours").fontWeight(.bold).foregroundColor(.orange)
                        }
                                ...
                                VStack(alignment: .leading, spacing: 8) {
                                    if ev.type == "Examen" {
                                        ...
                                        if !ev.description.isEmpty {
                                            Text("📝 Détails : \(ev.description)")
                                        }
                                    } else {
                                        Text("📚 \(ev.course)").font(.headline)
                                        if !ev.description.isEmpty {
                                            Text("🎯 Objectif : \(ev.description)")
                                        }
                ...
                if appData.courses.isEmpty {
                    ...
                    HStack(alignment: .top, spacing: 20) {
                        VStack(alignment: .leading) {
                            HStack {
                                ...
                                Spacer()
                                Button(action: { zoomedItem = .equilibre }) {
                        ...
                        VStack(alignment: .leading) {
                            HStack {
                                Text("Répartition du temps alloué").font(.headline)
                                Spacer()
                                Button(action: { zoomedItem = .repartition }) {
                                    Image(systemName: "plus.magnifyingglass")
                                }
                            }
                    ...
                    VStack(alignment: .leading) {
                        HStack {
                            ...
                            Spacer()
                            Button(action: { zoomedItem = .points }) {
                                Image(systemName: "plus.magnifyingglass")
                            }
                        }
        ...
        .sheet(item: $zoomedItem) { zItem in
            GeneralZoomModalView(appData: appData, zoomType: zItem)
        }
    }
}
    ...
    var body: some View {
        VStack {
            if let h = hoveredCourse {
                Text("\(h) : \(String(format: "%.1f", appData.computeProgress(for: h) * 100)) % accompli")
                    .font(.caption)
                    .bold()
...
struct GeneralRepartitionChart: View {
    ...
    var body: some View {
        let data = appData.courses.keys.map {
        ...
        VStack {
            if #available(macOS 14.0, *) {
                Text(hoverText)
                    .font(.caption)
                    .bold()
                    .foregroundColor(hoveredAngle == nil ? .secondary : .blue)
                
                Chart {
                    ForEach(data, id: \.name) { item in
                        SectorMark(
                            angle: .value("Jours", item.days),
                            innerRadius: .ratio(0.5),
                            angularInset: 1.5
                        )
                        .foregroundStyle(Color(hex: appData.courses[item.name]!.colorHex))
                        .annotation(position: .overlay) {
                            Text("\(item.days)j").font(.caption).bold().foregroundColor(.white)
                        }
                    }
                }
                .chartAngleSelection(value: $hoveredAngle)
            } else {
                Text("Nécessite macOS 14+")
            }
        }
    }
}
    ...
    var body: some View {
        ...
        VStack {
            if let h = hoveredCourse {
                ...
                Text("\(h) : \(String(format: "%.1f", acq)) acquis, \(String(format: "%.1f", aReu)) à réussir")
                    .font(.caption)
                    .bold()
                    .foregroundColor(.blue)
            } else {
...
struct GeneralZoomModalView: View {
    ...
    var body: some View {
        VStack {
            ...
            switch zoomType {
            case .equilibre:
                Text("Équilibre d'étude par cours").font(.title).bold()
                GeneralEquilibreChart(appData: appData).padding()
            case .repartition:
                Text("Répartition du temps alloué par cours").font(.title).bold()
                GeneralRepartitionChart(appData: appData).padding()
            case .points:
                Text("Stratégie des points (sur 20)").font(.title).bold()
                GeneralPointsChart(appData: appData).padding()
            }
            Spacer()
        }
        .padding()
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 30) {
                ...
                if appData.academicYears.isEmpty {
                    ...
                    HStack(spacing: 20) {
                        RecapCard(
                            title: "Crédits Validés (Toutes années)",
                            value: "\(String(format: "%.1f", stats.earned)) / \(String(format: "%.1f", stats.total))",
                            color: .green
                        )
                        RecapCard(
                            title: "Moyenne Globale Pondérée",
                            value: "\(String(format: "%.2f", stats.gpa)) / 20",
                            color: .blue
                        )
                        RecapCard(
                            title: "Cours passés uniques",
...
struct AcademicYearDetailView: View {
    ...
    var body: some View {
        if let year = appData.academicYears.first(where: { $0.id == yearId }) {
            let stats = appData.yearParcoursStats(yearId: year.id)
            ScrollView {
                VStack(alignment: .leading, spacing: 25) {
                    ...
                    HStack(spacing: 15) {
                        RecapCard(
                            title: "Crédits Validés sur l'année",
                            value: "\(String(format: "%.1f", stats.earned)) / \(String(format: "%.1f", stats.totalAttempted))",
                            color: .green
                        )
                        RecapCard(
                            title: "Moyenne Pondérée (Meilleure note)",
                            value: String(format: "%.2f", stats.gpa),
                            color: .blue
                        )
                        let avgAttempts = year.courses.isEmpty ? 0 : Double(year.courses.reduce(0) { $0 + $1.attempts }) / Double(year.courses.count)
                        RecapCard(
                            title: "Moyenne Tentatives (Base)",
                            value: String(format: "%.1f", avgAttempts),
                            color: .orange
                        )
                        RecapCard(
                            title: "Nouveaux cours inscrits",
                            value: "\(year.courses.count)",
                            color: .purple
                        )
                    }
                    Divider()
                    ...
                    VStack {
                        HStack {
                            Text("Premier Quadrimestre (Q1)").font(.title3).bold()
                            Spacer()
                            Button(action: { zoomedItem = .tableQ1 }) {
                                Image(systemName: "plus.magnifyingglass")
                            }
                        }
                    ...
                    VStack {
                        HStack {
                            Text("Deuxième Quadrimestre (Q2)").font(.title3).bold()
                            Spacer()
                            Button(action: { zoomedItem = .tableQ2 }) {
                                Image(systemName: "plus.magnifyingglass")
                            }
                        }
                        SemesterTableContent(appData: appData, yearId: year.id, courses: year.courses.filter { $0.semester == "Q2" })
                    }
                    Text("💡 Astuce : Double-cliquez sur un cours pour le modifier.")
                        .font(.caption)
                        .foregroundColor(.secondary)
                    ...
                    if !year.courses.isEmpty || !(year.exams ?? []).isEmpty {
                        ...
                        HStack(alignment: .top, spacing: 20) {
                            VStack(alignment: .leading) {
                                HStack {
                                    Text("Meilleure note par cours").font(.headline)
                                    Spacer()
                                    Button(action: { zoomedItem = .chartNotes }) {
                                        Image(systemName: "plus.magnifyingglass")
                                    }
                                }
                                NotesChart(appData: appData, year: year).frame(height: 250)
                            }
                            .padding()
                            .background(Color(NSColor.controlBackgroundColor))
                            .cornerRadius(12)
                            ...
                            if !year.courses.isEmpty {
                                VStack(alignment: .leading) {
                                    HStack {
                                        Text("Répartition des Crédits (PAE)").font(.headline)
                                        Spacer()
                                        Button(action: { zoomedItem = .chartCat }) {
                                            Image(systemName: "plus.magnifyingglass")
                                        }
                                    }
                        ...
                        if !(year.exams ?? []).isEmpty {
                            ...
                            HStack(alignment: .top, spacing: 15) {
                                ...
                                if !jan.isEmpty {
                                    VStack {
                                        HStack {
                                            Text("Janvier").font(.headline)
                                            Spacer()
                                            Button(action: { zoomedItem = .chartJan }) {
                                                Image(systemName: "plus.magnifyingglass")
                                            }
                                        }
                                ...
                                if !jun.isEmpty {
                                    VStack {
                                        HStack {
                                            Text("Juin").font(.headline)
                                            Spacer()
                                            Button(action: { zoomedItem = .chartJun }) {
                                                Image(systemName: "plus.magnifyingglass")
                                            }
                                        }
                                ...
                                if !aug.isEmpty {
                                    VStack {
                                        HStack {
                                            Text("Août").font(.headline)
                                            Spacer()
                                            Button(action: { zoomedItem = .chartAug }) {
                                                Image(systemName: "plus.magnifyingglass")
                                            }
                                        }
            ...
            .sheet(isPresented: $isShowingAddExam) {
                AddExamResultSheet(appData: appData, isPresented: $isShowingAddExam, yearId: year.id)
            }
            .sheet(item: $zoomedItem) { zoomType in
                ZoomModalView(appData: appData, year: year, zoomType: zoomType)
            }
        }
    }
}
...
struct SemesterTableContent: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            if courses.isEmpty {
                Text("Aucun cours encodé.").foregroundColor(.secondary).padding()
            } else {
                ForEach(courses) { course in
                    HStack {
                        ...
                        Button(action: { appData.removeParcoursCourse(yearId: yearId, courseId: course.id) }) {
                            Image(systemName: "trash").foregroundColor(.red)
                        }
                        .buttonStyle(.plain)
                        .frame(width: 30)
        ...
        .sheet(item: $courseToEdit) { course in
            EditParcoursCourseSheet(appData: appData, yearId: yearId, course: course)
        }
    }
}
...
struct ExamSessionSection: View {
    ...
    var body: some View {
        ...
        for ex in exams where ex.grade >= 10.0 {
            if let c = appData.getParcoursCourse(code: ex.courseCode) {
                credits += c.credits
            }
        }
        return VStack {
            HStack {
                ...
                Spacer()
                Text("Crédits validés à cette session : \(String(format: "%.1f", credits))")
                    .font(.subheadline)
...
struct ExamSessionTableContent: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            HStack {
                Text("Code").bold().frame(width: 100, alignment: .leading)
                Text("Nom du Cours").bold().frame(maxWidth: .infinity, alignment: .leading)
                Text("Note /20").bold().frame(width: 100, alignment: .center)
                Text("Tentative").bold().frame(width: 100, alignment: .center)
                Text("").frame(width: 30)
            }
            .padding(.horizontal, 10).padding(.vertical, 8).background(Color.orange.opacity(0.1))
            
            if exams.isEmpty {
                Text("Aucun résultat pour cette session.").foregroundColor(.secondary).padding()
            } else {
                ForEach(exams) { exam in
                    ...
                    HStack {
                        ...
                        Button(action: { appData.removeExamResult(yearId: yearId, examId: exam.id) }) {
                            Image(systemName: "trash").foregroundColor(.red)
        ...
        return VStack {
            ...
            Chart {
                ForEach(codesList, id: \.self) { code in
                    ...
                    BarMark(
                        x: .value("Cours", code),
                        y: .value("Note MAX", bestG)
                    )
                    .foregroundStyle(bestG >= 10.0 ? Color.blue : Color.red.opacity(0.6))
                    .opacity(hoveredCourse == nil || hoveredCourse == code ? 1.0 : 0.4)
                }
            }
            .chartXSelection(value: $hoveredCourse)
            .chartYScale(domain: 0...20)
    ...
    var body: some View {
        Group {
            if #available(macOS 14.0, *) {
                ...
                let data = groupedByCat.keys.map {
                ...
                let hoverText: String = {
                    ...
                    for item in data {
                        sum += item.credits
                        if angle <= sum { return "\(item.cat) : \(String(format: "%.1f", item.credits)) cr." }
                    }
                    return " "
                }()
                ...
                VStack {
                    ...
                    Chart {
                        ForEach(data, id: \.cat) { item in
                            SectorMark(
                                angle: .value("Crédits", item.credits),
                                innerRadius: .ratio(0.5),
                                angularInset: 1.5
                            )
                            .foregroundStyle(by: .value("Catégorie", item.cat))
                        }
                    }
                    .chartAngleSelection(value: $hoveredAngle)
                }
    ...
    var body: some View {
        VStack {
            ...
            case .examsJan:
                ...
                ExamSessionTableContent(appData: appData, yearId: year.id, exams: (year.exams ?? []).filter{ $0.sessionName == "Janvier" })
            case .examsJun:
                Text("Résultats Session Juin").font(.title).bold()
                ExamSessionTableContent(appData: appData, yearId: year.id, exams: (year.exams ?? []).filter{ $0.sessionName == "Juin" })
            case .examsAug:
                Text("Résultats Session Août").font(.title).bold()
                ExamSessionTableContent(appData: appData, yearId: year.id, exams: (year.exams ?? []).filter{ $0.sessionName == "Août" })
            case .chartNotes:
                Text("Notes MAX par cours").font(.title).bold()
                NotesChart(appData: appData, year: year).padding()
            case .chartCat:
...
struct EditParcoursCourseSheet: View {
    ...
    init(appData: AppData, yearId: UUID, course: ParcoursCourse) {
        ...
        _category = State(initialValue: course.category)
        _grade = State(initialValue: course.grade)
        _attempts = State(initialValue: course.attempts)
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 15) {
            ...
            HStack {
                VStack(alignment: .leading) {
                    Text("Crédits").font(.caption)
                    TextField("", value: $credits, format: .number).textFieldStyle(.roundedBorder)
                }
                VStack(alignment: .leading) {
                    Text("Note (/20)").font(.caption)
                    TextField("", value: $grade, format: .number).textFieldStyle(.roundedBorder)
                }
                VStack(alignment: .leading) {
                    Text("Tentatives").font(.caption)
            ...
            HStack {
                ...
                Button("Sauvegarder") {
                    if !code.isEmpty && !name.isEmpty {
                        ...
                        c.attempts = attempts
                        c.semester = semester
                        appData.updateParcoursCourse(yearId: yearId, course: c)
                        dismiss()
                    }
                }.buttonStyle(.borderedProminent).disabled(code.isEmpty || name.isEmpty).keyboardShortcut(.defaultAction)
            }.padding(.top, 10)
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 15) {
            ...
            Picker("Session", selection: $sessionName) {
                ForEach(sessions, id: \.self) { Text($0) }
            }.pickerStyle(.segmented)
            ...
            VStack(alignment: .leading) {
                ...
                Picker("", selection: $courseCode) {
                    ...
                    ForEach(appData.allParcoursCourses()) { c in
            ...
            HStack {
                VStack(alignment: .leading) {
                    Text("Note Obtenue (/20)").font(.caption)
                    TextField("", value: $grade, format: .number).textFieldStyle(.roundedBorder)
                }
                VStack(alignment: .leading) {
                    Text("Quantième tentative ?").font(.caption)
                    Stepper("\(attempt)", value: $attempt, in: 1...10)
                }
            }
            HStack {
                Button("Annuler") { isPresented = false }.keyboardShortcut(.cancelAction)
                ...
                Button("Ajouter") {
                    if !courseCode.isEmpty {
                        ...
                        appData.addExamResult(yearId: yearId, exam: newExam)
                        isPresented = false
                    }
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 20) {
                HStack {
                    ...
                    Spacer()
                    Button("🗑️ Vider TOUT le calendrier") { appData.schedule.removeAll() }.buttonStyle(.bordered)
                }
            ...
            LazyVGrid(columns: Array(repeating: GridItem(.flexible(), spacing: 5), count: 7), spacing: 5) {
                ...
                ForEach(0..<days.count, id: \.self) { i in
                    if let dayNum = days[i] {
                        CalendarCell(appData: appData, dayNum: dayNum, dateStr: String(format: "%04d-%02d-%02d", year, month, dayNum))
                    } else {
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 20) {
                        ...
                        VStack(alignment: .leading, spacing: 15) {
                            HStack(alignment: .bottom) {
                                VStack(alignment: .leading) {
                                    ...
                                    TextField("Ex: LINFO", text: $editedAcronym).textFieldStyle(.roundedBorder)
                                }
                            ...
                            HStack {
                                VStack(alignment: .leading) { Text("Nom complet"); TextField("Ex: Algorithmique", text: binding(for: \.fullName)) }
                                VStack(alignment: .leading) { Text("Professeur"); TextField("Ex: John Doe", text: binding(for: \.professor)) }
                            }
                            HStack {
                                VStack(alignment: .leading) { Text("Catégorie (Section)"); TextField("Ex: Tronc commun", text: Binding(get: { course.category ?? "Général" }, set: { appData.courses[courseName]?.category = $0.isEmpty ? nil : $0 })) }
                                VStack(alignment: .leading) { Text("Couleur"); ColorPicker("", selection: Binding(get: { Color(hex: course.colorHex) }, set: { appData.courses[courseName]?.colorHex = $0.toHex() })).labelsHidden() }
                                VStack(alignment: .leading) { Text("Cote cible (/20)"); TextField("", value: binding(for: \.passingGrade), format: .number).frame(width: 60) }
                            }
                            Text("Informations sur l'examen").bold()
                            HStack {
                                VStack(alignment: .leading) { Text("Début (HH:MM)"); TextField("", text: binding(for: \.examStartTime)).frame(width: 80) }
                                VStack(alignment: .leading) { Text("Fin (HH:MM)"); TextField("", text: binding(for: \.examEndTime)).frame(width: 80) }
                                VStack(alignment: .leading) { Text("Lieu"); TextField("", text: binding(for: \.examLocation)) }
                            }
                    ...
                    DisclosureGroup("🔗 Liens Rapides") {
                        VStack(alignment: .leading) {
                            HStack {
                                TextField("Nom (ex: Dossier Drive)", text: $newLinkName).textFieldStyle(.roundedBorder)
                                TextField("Lien URL", text: $newLinkUrl).textFieldStyle(.roundedBorder)
                                Button("Ajouter") {
                                    if !newLinkName.isEmpty && !newLinkUrl.isEmpty {
                                        var safeUrl = newLinkUrl
                                        if !safeUrl.lowercased().hasPrefix("http") { safeUrl = "https://" + safeUrl }
                            ...
                            if links.isEmpty {
                                Text("Aucun lien ajouté.").foregroundColor(.secondary).padding(.top, 5)
                            } else {
                                LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: 10) {
                                    ForEach(Array(links.enumerated()), id: \.element.id) { index, link in
                                        HStack {
                                            ...
                                            Spacer()
                                            Button("❌") { appData.courses[courseName]?.links?.remove(at: index) }.foregroundColor(.red).buttonStyle(.plain)
                                        }
                    ...
                    ForEach(Array(course.tasks.enumerated()), id: \.element.id) { index, task in
                        VStack(alignment: .leading) {
                            ...
                            HStack {
                                ...
                                Button("➕") { if task.done < task.total { appData.courses[courseName]?.tasks[index].done += 1; appData.registerActivity() } }
                    ...
                    ForEach(Array(course.grading.enumerated()), id: \.element.id) { index, grade in
                        HStack {
                            ...
                            TextField("Score", value: Binding(get: { grade.score }, set: { appData.courses[courseName]?.grading[index].score = $0 }), format: .number).textFieldStyle(.roundedBorder).frame(width: 60)
                    ...
                    if todos.isEmpty {
                        ...
                        ForEach(Array(todos.enumerated()), id: \.element.id) { index, todo in
                            HStack {
                                ...
                                TextField("Tâche", text: Binding(get: { todo.text }, set: { appData.courses[courseName]?.todos?[index].text = $0 })).strikethrough(todo.isDone).foregroundColor(todo.isDone ? .secondary : .primary)
                                if todo.dueDate != nil {
                                    DatePicker("", selection: Binding(get: { todo.dueDate ?? Date() }, set: { appData.courses[courseName]?.todos?[index].dueDate = $0 })).labelsHidden()
                                } else {
                                    Button(action: { appData.courses[courseName]?.todos?[index].dueDate = Date() }) { Image(systemName: "calendar.badge.plus").foregroundColor(.blue) }.buttonStyle(.plain).help("Ajouter une date")
                                }
                                Spacer()
